...
> **NOTE:** `EnableBedrockInvocationLogs` by default is set to `false` but are not enabled until an additional command is submitted via the AWS CLI. Learn more about [enabling Bedrock Model Invocation Logging account-wide](./bedrock-model-invocation-logging.md).
