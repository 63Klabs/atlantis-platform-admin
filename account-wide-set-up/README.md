...
> **NOTE:** `EnableBedrockInvocationLogs` by default is set to `false` due to the sensitive nature of the content. It should only be enabled (set to `true`) in certain contexts (mainly non-production accounts or accounts that don't receive user submitted prompts).  After setting to `true` it is still not enabled until an additional command is submitted via the AWS CLI. Learn more about [enabling Bedrock Model Invocation Logging account-wide](./bedrock-model-invocation-logging.md).
...
To skip the GitHub connection, set `GitHubOrg` to `""`.
